  ...
  return (
    <Link to={`/films/${film.id}`}>
      <div className="bg-surface border border-border p-1 hover:bg-accent-hover hover:shadow-lg transition-colors duration-300 rounded-lg overflow-hidden">
        ...
        <h2 className="font-semibold text-center text-text-primary px-2 pt-2">{film.title}</h2>
        <p className="text-text-secondary text-sm text-center pb-2">{film.release_date}</p>
      </div>
    </Link>
  );
}
